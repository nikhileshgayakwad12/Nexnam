  ...
  return (
    <div className={`relative group inline-flex items-center ${className}`}>
      <button
        ...
        className="w-11 h-11 rounded-full bg-white border border-slate-900/[0.08] text-[#0B0D12] hover:text-[#5B5CF6] hover:border-slate-300 shadow-[0_4px_16px_rgba(15,23,42,0.06)] hover:shadow-[0_8px_24px_rgba(15,23,42,0.1)] hover:-translate-y-0.5 active:scale-95 transition-all duration-200 flex items-center justify-center cursor-pointer pointer-events-auto focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5B5CF6]"
        aria-label={muted ? "Sound off. Click to turn sound on" : "Sound on. Click to turn sound off"}
      >
        {muted ? (
          <VolumeX className="w-5 h-5 text-slate-400 transition-colors" />
        ) : (
          <Volume2 className="w-5 h-5 text-slate-700 group-hover:text-[#5B5CF6] transition-colors" />
        )}
      </button>
